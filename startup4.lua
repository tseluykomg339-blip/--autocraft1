...
local craftCount = 1  -- <--- НОВОЕ: счётчик количества
...
function draw()
    if monControl then
        ...
        -- НОВОЕ: кнопки управления количеством
        ...
        -- Кнопка удаления (переехала на строку 8, чтобы не мешаться)
...
while true do
    ...
    if side == "right" then
        if y == 2 then 
            for i = 1, 9 do barrel.pushItems(turtleName, bSlots[i], craftCount, tSlots[i]) end  -- ИЗМЕНЕНО: 1 → craftCount
            rednet.send(turtleID, "CRAFT:" .. craftCount)  -- ИЗМЕНЕНО: передаём количество
        elseif y == 4 then
            ...
            save()
            draw()
        -- НОВОЕ: обработка кнопок количества
        elseif y == 6 then
                    ...
                    for slot, itemName in pairs(recipeData) do
                        for vSlot, item in pairs(vaultItems) do
                            if item.name == itemName then
                                storage.pushItems(turtleName, vSlot, craftCount, tSlots[slot])  -- ИЗМЕНЕНО: 1 → craftCount
                                break
                            end
                        end
                    end
